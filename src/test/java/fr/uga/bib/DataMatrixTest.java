...
public class DataMatrixTest {
    ...
    public void initCSV() throws IOException {
        dm = new DataFrame("src/test/resources/df1.csv");
    ...
    @Test
    public void initCSVEmptyDataFrame() throws IOException {
        dm = new DataFrame("src/test/resources/df2.csv");

        assertEquals("Get column number test", 3, dm.getNumCols());
        assertEquals("Get row number test", 0, dm.getNumRows());
    }

    @Test
    public void initCSVFileError() throws IOException {
        assertThrows("No type in the file",
                IOException.class,
                () -> dm = new DataFrame("src/test/resources/df3.csv")
        );
    }
    ...
    @Test
    public void addColumnLesser() {
        Object[] col = {1.0};

        dm.addColumn("sixth", Double.class, col);

        assertEquals("Column number incremented test", 6, dm.getNumCols());
        assertEquals("Row number unmodified test", 3, dm.getNumRows());
        assertEquals("Added column type test",
                Double.class,
                dm.getColumnType("sixth")
        );
        ...
        assertNull("Get third added value test",
                dm.getValue("sixth", 2, Double.class)
        );
    }
    ...
    public void testToString() {
        String msg = "first\tsecond\tthird\tfourth\tfifth\t\n" +
                "10\ttrue\tabc\t1.2\t1\t\n" +
                "20\tfalse\tdef\t5.3\t5\t\n" +
                "30\ttrue\tghi\t-100.02\t140\t\n";

        assertEquals("DataFrame string test", msg, dm.toString());
    }
}
